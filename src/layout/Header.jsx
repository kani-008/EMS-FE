...
const Header = () => {
  ...
  // Match the original staff uppercase display behavior
  const isStaff = user && user.role !== "STUDENT" && user.role !== "ADMIN";
  const displayLabel = isStaff ? displayName.toUpperCase() : displayName;

  // Resolve base path from role
  const basePath = user?.role === "STUDENT" ? "/student" : user?.role === "ADMIN" ? "/admin" : "/staff";
    ...
    <>
      <div className="header">
        ...
        <div className="relative flex items-center gap-3" ref={dropdownRef}>
          <span className="text-slate-600 font-semibold text-sm">{displayLabel}</span>
          <button
