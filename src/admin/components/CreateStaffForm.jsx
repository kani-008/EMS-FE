// frontend/src/admin/components/CreateStaffForm.jsx
...
const CreateStaffForm = ({ onClose, refreshUsers }) => {
    ...
    const load = async () => {
      try {
        const [deptRes, roleRes] = await Promise.all([
          ...
          fetch("/api/admin/staff-roles", { credentials: "include" }),
        ]);
  ...
  const validateBatch = useCallback(async (batchVal, courseVal) => {
    ...
    try {
      const res  = await fetch(
        `/api/admin/validate-batch?batch=${encodeURIComponent(batchVal)}&course=${encodeURIComponent(courseVal)}`,
        { credentials: "include" }
      );
      ...
      if (!res.ok || !data.success) {
        setBatchValid(false);
        setDerivedCurrentYear(null);
        setBatchError(data.message || "Batch validation failed");
        return;
      }
      ...
      setBatchValid(false);
      setDerivedCurrentYear(null);
      setBatchError("Could not reach validation service");
    } finally {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || "Failed to create staff user");
  ...
  const handleExcelUpload = async (e) => {
    ...
    try {
      ...
      const res  = await fetch("/api/admin/upload-staff-excel", {
        method:      "POST",
        credentials: "include",
        body:        formData,
      });
