...
const Table = ({
  ...
  const thtext = "text-xs text-base uppercase tracking-wide text-slate-500";
  ...
  return (
    <div
      className={`overflow-x-auto relative ${
        scrollable
          ? "overflow-y-auto max-h-[calc(100vh-220px)] pr-2"
          : "overflow-visible"
      }`}
    >
      <table
        className={`w-full table-fixed border-separate border-spacing-y-3 text-sm -pr-2 ${
          scrollable ? "" : "min-w-[900px]"
        }`}
      >
        <thead className={scrollable ? "sticky top-0 z-10 bg-[#f8fafc]" : ""}>
          <tr>
                  ...
                  <table className="w-full table-fixed">
                    ...
                    <tbody>
                      <tr>
                        ...
                        {columns.map((col, i) => (
                          <td key={i} className="px-2 py-3 whitespace-nowrap">
